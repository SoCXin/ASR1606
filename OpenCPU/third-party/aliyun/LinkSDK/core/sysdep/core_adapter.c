 ...
 */
#define CORE_ADAPTER_DTLS_ENABLED
typedef struct {
...
#ifndef CORE_ADAPTER_DTLS_ENABLED
...
int32_t _tls_network_establish(void *handle)
{
    ...
    if (adapter_handle->socket_type == CORE_SYSDEP_SOCKET_TCP_CLIENT) {
        ...
        mbedtls_ssl_set_timer_cb(&adapter_handle->mbedtls.ssl_ctx, (void *)&adapter_handle->mbedtls.timer_delay_ctx,
                                 _core_mbedtls_timing_set_delay, (mbedtls_ssl_get_timer_t *)_core_mbedtls_timing_get_delay);
#else
        return -1;
#endif
    }
    ...
    mbedtls_ssl_conf_min_version(&adapter_handle->mbedtls.ssl_config, MBEDTLS_SSL_MAJOR_VERSION_3,
                                 MBEDTLS_SSL_MINOR_VERSION_3);
    mbedtls_ssl_conf_rng(&adapter_handle->mbedtls.ssl_config, (int (*)(void *, unsigned char *, size_t))_core_mbedtls_random, NULL);
    mbedtls_ssl_conf_dbg(&adapter_handle->mbedtls.ssl_config, (void (*)(void *, int,  const char *, int,  const char *))_core_mbedtls_debug, stdout);
    ...
    if (adapter_handle->cred->option == AIOT_SYSDEP_NETWORK_CRED_SVRCERT_CA) {
        ...
        mbedtls_ssl_conf_ciphersuites(&adapter_handle->mbedtls.ssl_config, (const int *)ciphersuites);
    } else {
    ...
    mbedtls_ssl_set_bio(&adapter_handle->mbedtls.ssl_ctx, adapter_handle->network_handle, (mbedtls_ssl_send_t *)_core_mbedtls_net_send,
                        (mbedtls_ssl_recv_t *)_core_mbedtls_net_recv, (mbedtls_ssl_recv_timeout_t *)_core_mbedtls_net_recv_timeout);
