...
typedef struct errinfo_s {
	//lianghai debug
    unsigned int magic;             /* just in case */
...
#else
# ifdef __GNUC__
// entry here, __GUNU__ should be defined outside
#  define zprintf(level, format, args...) do {                          \
				if(_zbar_verbosity >= level) {									\
					qrPrintf(QR_LOG_INFO,"%s%s%s: " format, QR_LOG_MODULE_NAME, "[zprintf]", __func__, ##args);			\
					ZFLUSH														\
				}																\
			} while(0)
