...
static __inline unsigned calc_thresh (zbar_scanner_t *scn)
{
    ...
    if((thresh <= scn->y1_min_thresh) || !scn->width) {
        QR_DBG_PRINT(" tmin=%d", scn->y1_min_thresh);
    ...
    QR_DBG_PRINT(" thr=%d t=%ld x=%d last=%d.%d (%d)",
                                               ...
                                               int y1)
{
    ...
    QR_DBG_PRINT(" sgn=%d cur=%d.%d w=%d (%s)\n",
                                ...
                                int y)
{
    ...
    QR_DBG_PRINT("scan: x=%d y=%d y0=%d y1=%d y2=%d",
                 x, y, y0_1, y1_1, y2_1);
       ...
       (calc_thresh(scn) <= abs(y1_1)))
    {
        ...
        if(y1_rev || (abs(scn->y1_sign) < abs(y1_1))) {
            ...
            QR_DBG_PRINT("\tthr=%d", scn->y1_thresh);
