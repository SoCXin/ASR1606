...
static void symbol_handler (zbar_decoder_t *dcode)
{
    ...
    if(type <= ZBAR_PARTIAL) {
        QR_DBG_PRINT("partial symbol @(%d,%d)\n", x, y);
        return;
    }
    ...
    for(sym = iscn->syms->head; sym; sym = sym->next)
        if(sym->type == type &&
           ...
           !memcmp(sym->data, data, datalen)) {
            sym->quality++;
            QR_DBG_PRINT("dup symbol @(%d,%d): dup %s: %.20s\n",
    ...
    if(TEST_CFG(iscn, ZBAR_CFG_POSITION)) {
        QR_DBG_PRINT("new symbol @(%d,%d): %s: %.20s\n",
                x, y, zbar_get_symbol_name(type), data);
        sym_add_point(sym, x, y);
    }
...
{
    int i;
    debug("symbol sets allocated   = %-4d\n", iscn->stat_syms_new);
    debug("    scanner syms in use = %-4d\trecycled  = %-4d\n",
            iscn->stat_iscn_syms_inuse, iscn->stat_iscn_syms_recycle);
    debug("    image syms in use   = %-4d\trecycled  = %-4d\n",
            iscn->stat_img_syms_inuse, iscn->stat_img_syms_recycle);
    debug("symbols allocated       = %-4d\n", iscn->stat_sym_new);
    for(i = 0; i < RECYCLE_BUCKETS; i++)
        debug("     recycled[%d]        = %-4d\n",
                     ...
                     zbar_image_t *img)
{
    ...
    gstQrTime.timer2Start = qrGetTimeMs();
#ifdef ENABLE_QRCODE
    ...
    iscn->dx = 0;
    gstQrTime.timer2Stop = qrGetTimeMs();
	
    gstQrTime.timer3Start = qrGetTimeMs();
    ...
    gstQrTime.timer3Stop = qrGetTimeMs();
#ifdef ENABLE_QRCODE
    ...
    if(syms->nsyms) {
        ...
        if(nean == 1 && naddon == 1 && iscn->ean_config) {
            ...
            ean_sym = _zbar_image_scanner_alloc_sym(iscn, ZBAR_COMPOSITE, datalen);
